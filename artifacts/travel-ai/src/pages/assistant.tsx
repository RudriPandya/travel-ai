...
export default function Assistant() {
  ...
  const handleSend = () => {
    ...
    chatMutation.mutate({ data: { message: input } }, {
      onSuccess: () => {
        setInput("");
        // In a real app, invalidate query or optimistic update
      }
    });
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10">
          ...
          {history?.map((msg) => (
             <div key={msg.id} className={`flex gap-4 max-w-[80%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
               ...
               <div className={`p-4 rounded-2xl border ${msg.role === 'user' ? 'bg-primary/20 border-primary/30 rounded-tr-sm text-primary-foreground' : 'bg-muted/50 border-border rounded-tl-sm'}`}>
